<?php
		...
		if(!isset($_COOKIE["login"])){
			...
			if(empty($_REQUEST["gender"]) || empty($_REQUEST["caste"]) || empty($_REQUEST["occupation"]) || empty($_REQUEST["mstatus"])){
				...
				include("db.php"); 
				$rp=mysqli_query($conn,"select count(*) from user where gender='$gender' AND caste='$caste' AND occupation='$occupation' AND mstatus='$mstatus' AND email<>'$email'");
				if($r=mysqli_fetch_array($rp)){
					$count=$r[0];
				}
				$page=ceil($count/2);
				if($id<0 & $id>=$page){
			    ...
			    $flag=0;
				$rs=mysqli_query($conn,"select * from user where gender='$gender' AND caste='$caste' AND occupation='$occupation' AND mstatus='$mstatus' AND email<>'$email' limit $start,2");
  ...
  <div class="container">
   ...
   <div class="profile">
				...
				<table class="table_working_hours">
		        	<tbody>
				      ...
				      <div class="buttons">   
						    ...
						    <a href="full_profile.php?code=<?php echo $r["code"]?>"><div class="vertical">Full Details</div></a>
						    <span class="interest" style="cursor:pointer" id="<?php echo $r["code"]?>"><div class="vertical">Send Interest</div></span>
					</div>	    
			</div>
		</div>	
	 ...
	 <?php
			...
			if($id>0){
			?>
			<a href="search_profile.php?id=<?php echo $id-1;?>&gender=<?php echo $gender;?>&caste=<?php echo $caste;?>&occupation=<?php echo $occupation;?>&mstatus=<?php echo $mstatus;?>"><button class="btn btn-info">Previous</button></a>
			<?php
			}
			if($id!=$page-1){
				
					?>
				<a href="search_profile.php?id=<?php echo $id+1;?>&gender=<?php echo $gender;?>&caste=<?php echo $caste;?>&occupation=<?php echo $occupation;?>&mstatus=<?php echo $mstatus;?>"><button class="btn btn-info" style="float:right">Next</button></a>
